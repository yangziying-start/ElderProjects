      ...
      <el-tab-pane label="值班排班" name="duty">
        <el-card>
          ...
          <el-table :data="dutyList" v-loading="dutyLoading">
            ...
            <el-table-column prop="status" label="状态" width="80">
              <template #default="{ row }">
                <el-tag :type="getDutyStatusType(row)" size="small">{{ getDutyStatusText(row) }}</el-tag>
              </template>
            </el-table-column>
...
// 根据值班日期和时间计算实际状态
const getDutyStatusText = (row) => {
  if (row.status === 0) return '休息'
  
  const now = new Date()
  const today = now.toISOString().split('T')[0] // YYYY-MM-DD
  const dutyDate = row.dutyDate
  
  // 值班日期在今天之后 -> 未开始
  if (dutyDate > today) return '未开始'
  
  // 值班日期在今天之前
  if (dutyDate < today) {
    // 夜间值班特殊处理：如果是昨天的夜间值班，可能跨到今天
    if (row.dutyType === 2) {
      const yesterday = new Date(now)
      yesterday.setDate(yesterday.getDate() - 1)
      const yesterdayStr = yesterday.toISOString().split('T')[0]
      if (dutyDate === yesterdayStr) {
        // 检查当前时间是否在夜间值班的次日时段内（00:00 - endTime）
        const currentTime = now.toTimeString().slice(0, 8)
        if (currentTime <= row.endTime) return '值班中'
      }
    }
    return '已结束'
  }
  
  // 值班日期是今天，检查当前时间是否在值班时间段内
  const currentTime = now.toTimeString().slice(0, 8) // HH:mm:ss
  
  if (row.dutyType === 2) {
    // 夜间值班：20:00:00 - 08:00:00（跨天）
    // 今天的夜间值班，当前时间 >= startTime 就是值班中
    if (currentTime >= row.startTime) return '值班中'
    return '未开始'
  } else {
    // 日间值班：正常时间段比较
    if (currentTime < row.startTime) return '未开始'
    if (currentTime > row.endTime) return '已结束'
    return '值班中'
  }
}

const getDutyStatusType = (row) => {
  const statusText = getDutyStatusText(row)
  const typeMap = {
    '值班中': 'success',
    '未开始': 'warning',
    '已结束': 'info',
