  ...
  <div class="meal-manage">
    ...
    <el-dialog v-model="scheduleDialogVisible" :title="scheduleForm.id ? '编辑值班' : '新增值班'" width="500px">
      <el-form :model="scheduleForm" label-width="100px">
        ...
        <el-form-item label="配送员">
          <el-select v-model="scheduleForm.workerId" placeholder="请选择配送员" style="width: 100%;" @change="onWorkerChange">
            <el-option v-for="w in deliveryWorkers" :key="w.id" :label="`${w.name} (${w.phone})`" :value="w.id">
              <span>{{ w.name }} ({{ w.phone }})</span>
              <span v-if="w.buildings" style="color: #909399; margin-left: 8px; font-size: 12px;">- {{ w.buildings }}</span>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="负责楼栋">
          <el-tag v-if="selectedWorkerBuildings" type="success">{{ selectedWorkerBuildings }}</el-tag>
          <el-text v-else type="info">请先选择配送员（楼栋信息来自楼栋管理中的绑定关系）</el-text>
        </el-form-item>
    ...
    <el-dialog v-model="batchScheduleDialogVisible" title="批量排班" width="500px">
      <el-form :model="batchScheduleForm" label-width="100px">
        ...
        <el-form-item label="配送员">
          <el-select v-model="batchScheduleForm.workerId" placeholder="请选择配送员" style="width: 100%;" @change="onBatchWorkerChange">
            <el-option v-for="w in deliveryWorkers" :key="w.id" :label="`${w.name} (${w.phone})`" :value="w.id">
              <span>{{ w.name }} ({{ w.phone }})</span>
              <span v-if="w.buildings" style="color: #909399; margin-left: 8px; font-size: 12px;">- {{ w.buildings }}</span>
            </el-option>
        ...
        <el-form-item label="负责楼栋">
          <el-tag v-if="batchSelectedWorkerBuildings" type="success">{{ batchSelectedWorkerBuildings }}</el-tag>
          <el-text v-else type="info">请先选择配送员（楼栋信息来自楼栋管理中的绑定关系）</el-text>
        </el-form-item>
      </el-form>
      <template #footer>
...
const scheduleForm = reactive({ id: null, scheduleDate: '', mealType: null, workerId: null, remark: '' })
const batchScheduleDialogVisible = ref(false)
const batchScheduleForm = reactive({ dateRange: [], mealType: null, workerId: null })
const selectedWorkerBuildings = ref('')
const batchSelectedWorkerBuildings = ref('')
...
const loadWorkerSchedules = async () => {
  ...
  try {
    ...
    const weekDate = new Date(scheduleWeek.value);
    const day = weekDate.getDay(); // 周日为0
    const startDate = new Date(weekDate);
    startDate.setDate(weekDate.getDate() - day); // 计算本周日
    const endDate = new Date(startDate);
    endDate.setDate(startDate.getDate() + 6); // 计算下周六
...
const handleAddSchedule = () => {
  Object.assign(scheduleForm, { id: null, scheduleDate: '', mealType: null, workerId: null, remark: '' })
  selectedWorkerBuildings.value = ''
  scheduleDialogVisible.value = true
}

const handleEditSchedule = (row) => {
  Object.assign(scheduleForm, { id: row.id, scheduleDate: row.scheduleDate, mealType: row.mealType, workerId: row.workerId, remark: row.remark })
  // 查找配送员绑定的楼栋
  const worker = deliveryWorkers.value.find(w => w.id === row.workerId)
  selectedWorkerBuildings.value = worker?.buildings || '全部楼栋'
  scheduleDialogVisible.value = true
}

const onWorkerChange = (workerId) => {
  const worker = deliveryWorkers.value.find(w => w.id === workerId)
  selectedWorkerBuildings.value = worker?.buildings || '全部楼栋'
}

const onBatchWorkerChange = (workerId) => {
  const worker = deliveryWorkers.value.find(w => w.id === workerId)
  batchSelectedWorkerBuildings.value = worker?.buildings || '全部楼栋'
}
...
const submitSchedule = async () => {
  ...
  try {
    ...
    if (scheduleForm.id) {
      await request.put(`/admin/meal/worker-schedules/${scheduleForm.id}`, data)
    } else {
      await request.post('/admin/meal/worker-schedules', data)
    }
    ElMessage.success('保存成功')
...
const handleBatchAddSchedule = () => {
  Object.assign(batchScheduleForm, { dateRange: [], mealType: null, workerId: null })
  batchSelectedWorkerBuildings.value = ''
