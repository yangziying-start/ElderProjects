...
const checkScheduleConflict = () => {
  const { workerId, scheduleDate, startTime, endTime, id } = scheduleForm
  if (!workerId || !scheduleDate || !startTime || !endTime) return null
  ...
  for (const schedule of scheduleList.value) {
    // 跳过当前编辑的排班
    if (id && schedule.id === id) continue
    // 检查是否同一保洁员同一天
    if (schedule.workerId === workerId && schedule.scheduleDate === scheduleDate) {
      const existStart = toMinutes(schedule.startTime)
      const existEnd = toMinutes(schedule.endTime)
      // 检查时间段是否重叠
      if (!(newEnd <= existStart || newStart >= existEnd)) {
        return `该保洁员在 ${scheduleDate} ${schedule.startTime}-${schedule.endTime} 已有排班`
      }
    }
  }
  return null
