...
public class AdminMealController {
    ...
    private void log(String token, String description) {
        try {
            ...
            if (user != null && user.getUserType() == 4) {
                adminLogService.log(userId, user.getName(), "餐饮管理", description);
            }
    ...
    public Result<MealWeeklyMenu> addWeeklyMenu(@RequestBody MealWeeklyMenu menu,
        ...
        log(token, "新增周食谱: " + menu.getWeekStartDate() + " ~ " + menu.getWeekEndDate());
    ...
    public Result<Void> publishWeeklyMenu(@PathVariable Long id,
        ...
        log(token, "发布周食谱: " + (existing != null ? existing.getWeekStartDate() + " ~ " + existing.getWeekEndDate() : "ID=" + id));
        return Result.success();
    }
    ...
    public Result<Void> deleteWeeklyMenu(@PathVariable Long id,
        ...
        log(token, "删除周食谱: " + (menu != null ? menu.getWeekStartDate() + " ~ " + menu.getWeekEndDate() : "ID=" + id));
        return Result.success();
    }
    ...
    public Result<MealDailyDish> addDish(@RequestBody MealDailyDish dish,
        ...
        log(token, "新增菜品: " + dish.getDishName());
    ...
    public Result<Void> updateDish(@PathVariable Long id, @RequestBody MealDailyDish dish,
        ...
        log(token, "修改菜品: " + dish.getDishName());
        return Result.success();
    }
    ...
    public Result<Void> deleteDish(@PathVariable Long id,
        ...
        log(token, "删除菜品: " + (dish != null ? dish.getDishName() : "ID=" + id));
        return Result.success();
    }
    ...
    public Result<Void> updateDishStatus(@PathVariable Long id, @RequestBody Map<String, Integer> body,
                                         @RequestHeader("Authorization") String token) {
        MealDailyDish existing = dailyDishMapper.selectById(id);
        MealDailyDish dish = new MealDailyDish();
        dish.setId(id);
        dish.setStatus(body.get("status"));
        dailyDishMapper.updateById(dish);
        String action = body.get("status") == 1 ? "启用" : "禁用";
        log(token, action + "菜品: " + (existing != null ? existing.getDishName() : "ID=" + id));
        return Result.success();
    }
    ...
    public Result<Void> updateOrderStatus(@PathVariable Long id, @RequestBody Map<String, Integer> body,
        ...
        String[] statusNames = {"待配送", "配送中", "已完成", "已取消"};
        int status = body.get("status");
        String statusName = status >= 0 && status < statusNames.length ? statusNames[status] : "状态" + status;
        log(token, "修改餐饮订单状态: 订单ID=" + id + ", 状态=" + statusName);
        return Result.success();
    }
    ...
    public Result<MealDishTemplate> addDishTemplate(@RequestBody MealDishTemplate template,
        ...
        log(token, "新增菜品模板: " + template.getDishName());
    ...
    public Result<Void> updateDishTemplate(@PathVariable Long id, @RequestBody MealDishTemplate template,
        ...
        log(token, "修改菜品模板: " + template.getDishName());
        return Result.success();
    }
    ...
    public Result<Void> deleteDishTemplate(@PathVariable Long id,
        ...
        log(token, "删除菜品模板: " + (template != null ? template.getDishName() : "ID=" + id));
        return Result.success();
    }
