...
public class AdminMealController {
    ...
    public Result<List<Map<String, Object>>> listDeliveryWorkers() {
        ...
        for (User worker : workers) {
            ...
            workerInfo.put("name", worker.getName());
            workerInfo.put("phone", worker.getPhone());
            
            // 查询该配送员绑定的楼栋
            List<BuildingWorker> buildingWorkers = buildingWorkerMapper.selectList(
                    new LambdaQueryWrapper<BuildingWorker>()
                            .eq(BuildingWorker::getWorkerId, worker.getId())
                            .eq(BuildingWorker::getStatus, 1));
            
            if (!buildingWorkers.isEmpty()) {
                String buildings = buildingWorkers.stream()
                        .map(BuildingWorker::getBuilding)
                        .collect(java.util.stream.Collectors.joining(","));
                workerInfo.put("buildings", buildings);
            } else {
                workerInfo.put("buildings", null);
            }
     ...
     * 根据楼栋获取分配的配送员
     */
    @GetMapping("/delivery-workers/by-building")
    public Result<List<Map<String, Object>>> getWorkersByBuilding(@RequestParam String building) {
        ...
        List<Map<String, Object>> result = new ArrayList<>();
        for (BuildingWorker bw : buildingWorkers) {
            User worker = userMapper.selectById(bw.getWorkerId());
            if (worker != null && worker.getStatus() == 1) {
                Map<String, Object> map = new HashMap<>();
                map.put("id", worker.getId());
                map.put("name", worker.getName());
                map.put("phone", worker.getPhone());
                map.put("isPrimary", bw.getIsPrimary());
                result.add(map);
            }
        }
        return Result.success(result);
     ...
     * 获取所有楼栋及其配送员（用于值班安排）
     */
    @GetMapping("/buildings-with-workers")
    public Result<List<Map<String, Object>>> getBuildingsWithWorkers() {
        // 获取所有启用的楼栋
    ...
    @Log(module = "餐饮管理", operationType = 1, desc = "'新增周食谱: ' + #menu.weekStartDate + '~' + #menu.weekEndDate")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    ...
    @Log(module = "餐饮管理", operationType = 2, desc = "'修改周食谱 ID: ' + #id")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    ...
    @Log(module = "餐饮管理", operationType = 2, desc = "'发布周食谱 ID: ' + #id")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    public Result<Void> publishWeeklyMenu(@PathVariable Long id) {
        ...
        weeklyMenuMapper.updateById(menu);
        MealWeeklyMenu existing = weeklyMenuMapper.selectById(id);
        return Result.success();
    }

    @DeleteMapping("/weekly-menus/{id}")
    @Log(module = "餐饮管理", operationType = 3, desc = "'删除周食谱 ID: ' + #id")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    ...
    @PostMapping("/dishes")
    @Log(module = "餐饮管理", operationType = 1, desc = "'新增菜品: ' + #dish.dishName")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    ...
    @Log(module = "餐饮管理", operationType = 2, desc = "'修改菜品: ' + #dish.dishName")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    ...
    @Log(module = "餐饮管理", operationType = 3, desc = "'删除菜品 ID: ' + #id")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    public Result<Void> deleteDish(@PathVariable Long id) {
        MealDailyDish dish = dailyDishMapper.selectById(id);
        dailyDishMapper.deleteById(id);
        return Result.success();
    }

    @PostMapping("/dishes/{id}/status")
    @Log(module = "餐饮管理", operationType = 2, desc = "'修改菜品状态 ID: ' + #id + ' 状态: ' + (#body['status']==1?'启用':'禁用')")
    ...
    @Log(module = "餐饮管理", operationType = 2, desc = "'修改配送配置 ID: ' + #id")
    ...
    @Log(module = "餐饮管理", operationType = 2, desc = "'修改餐饮订单状态 ID: ' + #id + ' 新状态: ' + #body['status']")
    ...
    @Log(module = "餐饮管理", operationType = 1, desc = "'新增菜品模板: ' + #template.dishName")
    ...
    @Log(module = "餐饮管理", operationType = 2, desc = "'修改菜品模板: ' + #template.dishName")
    ...
    @Log(module = "餐饮管理", operationType = 3, desc = "'删除菜品模板 ID: ' + #id")
    public Result<Void> deleteDishTemplate(@PathVariable Long id) {
        MealDishTemplate template = dishTemplateMapper.selectById(id);
    ...
    @Log(module = "餐饮管理", operationType = 1, desc = "'从模板添加菜品, 菜单ID: ' + #menuId + ' 模板ID: ' + #body['templateId']")
    @CacheEvict(value = "weekly_menu", allEntries = true)
    ...
    @Log(module = "餐饮管理", operationType = 1, desc = "'新增配送排班, 员工ID: ' + #schedule.workerId")
    public Result<MealWorkerSchedule> addWorkerSchedule(@RequestBody MealWorkerSchedule schedule) {
        ...
        // 自动从配送员-楼栋绑定关系获取负责的楼栋
        List<BuildingWorker> buildingWorkers = buildingWorkerMapper.selectList(
                new LambdaQueryWrapper<BuildingWorker>()
                        .eq(BuildingWorker::getWorkerId, schedule.getWorkerId())
                        .eq(BuildingWorker::getStatus, 1));
        if (!buildingWorkers.isEmpty()) {
            String buildings = buildingWorkers.stream()
                    .map(BuildingWorker::getBuilding)
            ...
            schedule.setBuildings(null); // 未绑定楼栋则为空
        }
    ...
    @Log(module = "餐饮管理", operationType = 1, desc = "'批量新增配送排班, 员工ID: ' + #body['workerId']")
    public Result<Void> batchAddWorkerSchedule(@RequestBody Map<String, Object> body) {
        ...
        if (!buildingWorkers.isEmpty()) {
            buildings = buildingWorkers.stream()
                    .map(BuildingWorker::getBuilding)
                    .collect(java.util.stream.Collectors.joining(","));
        }
    ...
    @Log(module = "餐饮管理", operationType = 2, desc = "'修改配送排班 ID: ' + #id")
    ...
    @Log(module = "餐饮管理", operationType = 3, desc = "'删除配送排班 ID: ' + #id")
