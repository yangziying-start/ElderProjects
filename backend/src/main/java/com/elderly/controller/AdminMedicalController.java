...
public class AdminMedicalController {
    ...
    private void log(String token, String description) {
        try {
            ...
            if (user != null && user.getUserType() == 4) {
                adminLogService.log(userId, user.getName(), "医疗管理", description);
            }
    ...
    public Result<Void> deleteDoctor(@PathVariable Long id,
        ...
        log(token, "删除医生: " + (doctor != null ? doctor.getName() : "ID=" + id));
        return Result.success();
    }
