...
public class WorkerController {
    ...
    public Result<List<OrderDetailVO>> getAvailableOrders(
        ...
        // 同一楼栋的所有配送员都能看到该楼栋的工单
        if ((effectiveType == null && workerType != null && workerType == 1) || "meal".equals(effectiveType)) {
            // 获取当前配送员负责的楼栋列表
            List<String> workerBuildings = buildingWorkerMapper.selectBuildingsByWorkerId(userId);
            
            List<MealOrder> mealOrders;
            if (workerBuildings != null && !workerBuildings.isEmpty()) {
                // 先查询所有待配送且未接单的订单
                List<MealOrder> allPendingOrders = mealOrderMapper.selectList(
                        new LambdaQueryWrapper<MealOrder>()
                                .eq(MealOrder::getStatus, 0)
                                .isNull(MealOrder::getWorkerId)
                                .ge(MealOrder::getDishDate, today)
                                .eq(MealOrder::getDeleted, 0)
                                .orderByAsc(MealOrder::getDishDate));
                
                // 过滤出地址包含配送员负责楼栋的订单
                mealOrders = allPendingOrders.stream()
                        .filter(order -> {
                            if (order.getAddress() == null) return false;
                            for (String building : workerBuildings) {
                                if (order.getAddress().contains(building)) {
                                    return true;
                                }
                            }
                            return false;
                ...
                // 如果没有绑定楼栋，则只显示直接分配给自己的订单
                mealOrders = mealOrderMapper.selectList(
                        new LambdaQueryWrapper<MealOrder>()
    ...
    public Result<Map<String, Object>> getStats(@RequestAttribute Long userId) {
        ...
        // 配送员(workerType=1)统计 - 通过订单地址匹配楼栋
        if (workerType == null || workerType == 1) {
            List<String> workerBuildings = buildingWorkerMapper.selectBuildingsByWorkerId(userId);
            
            if (workerBuildings != null && !workerBuildings.isEmpty()) {
                // 查询所有待配送且未接单的订单，然后按地址过滤
                List<MealOrder> allPendingOrders = mealOrderMapper.selectList(
                        new LambdaQueryWrapper<MealOrder>()
                                .eq(MealOrder::getStatus, 0)
                                .isNull(MealOrder::getWorkerId)
                                .eq(MealOrder::getDeleted, 0));
                pendingMeal = allPendingOrders.stream()
                        .filter(order -> {
                            if (order.getAddress() == null) return false;
                            for (String building : workerBuildings) {
                                if (order.getAddress().contains(building)) return true;
                            }
                            return false;
                        }).count();
    ...
    public Result<OrderDetailVO> getOrderDetail(
        ...
        switch (type) {
            case "meal":
                ...
                if (!canWorkerHandleMealOrder(userId, mealOrder)) {
    ...
    public Result<Map<String, Object>> acceptOrder(
        ...
        switch (type) {
            case "meal":
                ...
                // 用 workerId 判断是否已接单（餐饮订单创建时就有服务码）
                if (mealOrder.getWorkerId() != null) {
                    throw new RuntimeException("订单已接单");
                }
                mealOrder.setWorkerId(userId); // 接单时绑定配送员
                mealOrder.setUpdateTime(LocalDateTime.now());
                mealOrderMapper.updateById(mealOrder);
                // 返回订单原有的服务码
                result.put("serviceCode", mealOrder.getServiceCode());
                return Result.success(result);
    ...
    /**
     * 检查配送员是否有权限处理某个餐饮订单
     * 规则：订单地址包含配送员负责的楼栋名称，就有权限
     */
    private boolean canWorkerHandleMealOrder(Long workerId, MealOrder mealOrder) {
        // 如果订单已经分配给该配送员，直接返回true
        if (workerId.equals(mealOrder.getWorkerId())) {
            return true;
        }
        
        // 获取配送员负责的楼栋列表
        List<String> workerBuildings = buildingWorkerMapper.selectBuildingsByWorkerId(workerId);
        if (workerBuildings == null || workerBuildings.isEmpty()) {
            // 如果没有绑定楼栋，则只能处理直接分配给自己的订单
            return false;
        }
        
        // 通过订单地址匹配楼栋
        String address = mealOrder.getAddress();
    ...
    private OrderDetailVO convertMealToDetail(MealOrder order) {
        ...
        // 处理合并订单：如果remark以"菜品:"开头，说明是合并订单，显示remark中的菜品信息
