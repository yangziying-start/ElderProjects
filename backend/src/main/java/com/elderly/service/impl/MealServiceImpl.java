...
public class MealServiceImpl implements MealService {
    ...
    public Map<String, Object> createOrder(MealBookingRequest request, Long userId) {
        ...
        // 自动分配配送员：优先使用值班安排，其次根据用户楼栋查找负责的配送员
        Long assignedWorkerId = autoAssignWorker(elderlyId, request.getDishDate(), request.getMealType());
     ...
     * 根据用户楼栋和值班安排自动分配配送员
     */
    private Long autoAssignWorker(Long elderlyId, LocalDate dishDate, Integer mealType) {
        // 1. 获取用户的家庭信息
        Family family = familyMapper.selectByUserId(elderlyId);
        String building = family != null ? family.getBuilding() : null;
        
        // 2. 优先查找当天值班的配送员
        List<MealWorkerSchedule> schedules = workerScheduleMapper.selectByDateAndMealType(dishDate, mealType);
        if (!schedules.isEmpty()) {
            // 如果有指定楼栋的值班配送员，优先使用
            if (building != null) {
                for (MealWorkerSchedule schedule : schedules) {
                    if (schedule.getBuildings() != null && !schedule.getBuildings().isEmpty()) {
                        String[] buildings = schedule.getBuildings().split(",");
                        for (String b : buildings) {
                            if (b.trim().equals(building)) {
                                return schedule.getWorkerId();
                            }
                        }
                    }
                }
            }
            // 如果没有指定楼栋的，使用第一个值班配送员
            for (MealWorkerSchedule schedule : schedules) {
                if (schedule.getBuildings() == null || schedule.getBuildings().isEmpty()) {
                    return schedule.getWorkerId();
                }
            ...
            // 如果都指定了楼栋但没匹配上，使用第一个
            return schedules.get(0).getWorkerId();
        }
        ...
        // 4. 获取该楼栋的主要配送员
        BuildingWorker primaryWorker = buildingWorkerMapper.selectPrimaryByBuilding(building);
        if (primaryWorker != null) {
            return primaryWorker.getWorkerId();
        }
        
        // 5. 如果没有主要配送员，获取该楼栋任意一个配送员
        List<BuildingWorker> workers = buildingWorkerMapper.selectByBuilding(building);
        if (!workers.isEmpty()) {
