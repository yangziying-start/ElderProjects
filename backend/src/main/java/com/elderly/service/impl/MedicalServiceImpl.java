...
public class MedicalServiceImpl implements MedicalService {
    ...
    public List<DoctorListVO> getAvailableDoctors(LocalDate date) {
        ...
        for (MedicalDoctorDuty duty : duties) {
            ...
            // 根据值班时间段动态设置状态
            int dutyStatus = isToday ? (isInDutyTime ? 1 : 0) : duty.getStatus();
            ...
            // 夜间急诊不设熔断限制，但需要在值班时间内
            if (vo.getIsNightShift()) {
                vo.setIsOpen(false);
                // 夜间急诊：如果是今天，需要在值班时间内；如果是未来日期，可以预约
                vo.setCanBook(!isToday || isInDutyTime);
            } else {
                // 日间模式：检查是否在值班时间段内和熔断状态
                vo.setIsOpen(isCircuitOpen);
                // 如果是今天，需要在值班时间内；如果是未来日期，可以预约
                vo.setCanBook((!isToday || isInDutyTime) && !isCircuitOpen && queueCount < doctor.getMaxQueueLimit());
            }
    ...
    public void updateCircuitBreakerStatus(Long doctorId) {
        ...
        if (isNightShift) {
            // 夜间急诊不设熔断
            isOpen = 0;
        } else if (duty == null || !isInDutyTime) {
            // 非值班时段，触发熔断停止接单
            isOpen = 1;
