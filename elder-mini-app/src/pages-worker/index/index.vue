<template>
  <view class="container">
    ...
    <view class="quick-entry">
      ...
      <view class="entry-item" @click="goPage('/pages-worker/task/list')">
        <text class="entry-icon">📋</text>
        <text class="entry-text">接单大厅</text>
      </view>
    ...
    <view class="task-list">
      <view class="task-card" v-for="task in pendingTasks" :key="task.id" @click="goTaskDetail(task)">
        ...
        <view class="task-actions">
          <view class="action-btn" v-if="task.status === 0" @click.stop="goExecute(task)">开始服务</view>
          <view class="action-btn green" v-if="task.status === 1" @click.stop="goExecute(task)">继续服务</view>
        </view>
      </view>
    ...
    <view class="tab-bar">
      ...
      <view class="tab-item" @click="goPage('/pages-worker/user/index')">
        <text class="tab-icon">👤</text>
...
onShow(async () => {
  try {
    ...
    // 获取待服务和服务中的任务
    const myOrders = await get('/worker/my-orders') || []
    pendingTasks.value = myOrders.filter(o => o.status === 0 || o.status === 1).slice(0, 5)
  } catch (e) {
    console.error(e)
  }
})
