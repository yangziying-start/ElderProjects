<template>
  <view class="container">
    ...
    <view class="action-section">
      ...
      <view class="action-btn nav" @click="openNavigation">🗺️ 导航前往</view>
    </view>
  </view>
</template>
...
const callElderly = () => {
  if (task.value.elderlyPhone) {
    uni.makePhoneCall({ phoneNumber: task.value.elderlyPhone })
  } else {
...
const openNavigation = () => {
  if (task.value.address) {
    uni.openLocation({
      latitude: 39.908823,
      longitude: 116.397470,
      name: task.value.address,
...
.action-btn.nav { background: #fff7e6; color: #fa8c16; border: 2rpx solid #ffc53d; }
</style>