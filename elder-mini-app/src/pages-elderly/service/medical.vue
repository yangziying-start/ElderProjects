        ...
        >
          ...
          <view class="doctor-status">
            ...
            <view class="duty-info">
              <view class="duty-status-tag" :class="{ active: doc.dutyStatus === 1 }">
                {{ doc.dutyStatus === 1 ? '值班中' : '非值班' }}
              </view>
              <text class="duty-time">{{ doc.startTime }} - {{ doc.endTime }}</text>
            </view>
          </view>
        ...
        <view class="modal-body">
          ...
          <view class="form-item">
            ...
            <view class="address-row">
              <picker mode="selector" :range="buildingList" range-key="building" @change="onBuildingChange">
                <view class="picker-input">
                  {{ bookingForm.building || '选择楼栋' }}
...
const loadBuildingList = async () => {
  try {
    const data = await get('/service/buildings')
...
const onBuildingChange = (e) => {
  ...
  bookingForm.value.building = buildingList.value[index]?.building || ''
}
...
const getBookBtnText = (doc) => {
  ...
  if (!doc.canBook) {
    if (doc.currentQueueCount >= doc.maxQueueLimit) {
      return '已满'
    }
    return '非值班时段'
  }
...
const bookDoctor = (doc) => {
  if (!doc.canBook) {
    ...
    if (doc.isOpen) {
      ...
      msg = `非值班时段（值班时间 ${doc.startTime}-${doc.endTime}）`
    }
...
const submitBooking = async () => {
  ...
  // 组合地址
  const address = bookingForm.value.building 
    ? (bookingForm.value.room ? `${bookingForm.value.building} ${bookingForm.value.room}` : bookingForm.value.building) 
    : ''
  ...
  try {
    ...
    bookingForm.value = { symptoms: '', building: '', room: '', remark: '' }
    await loadDoctors()
...
.duty-info { display: flex; align-items: center; gap: 12rpx; }
.duty-status-tag {
  font-size: 22rpx;
  padding: 4rpx 12rpx;
  border-radius: 8rpx;
